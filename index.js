...
function dateToUTC(date) {
  ...
  const month = date.getUTCMonth() + 1; // Місяці в Date починаються з 0, тому додаємо 1.
...
function nextNewYear() {
  ...
  return `До наступного Нового року залишилось: ${days} днів, ${hours} годин, ${minutes} хвилин, ${seconds} секунд, ${milliseconds} мілісекунд`;
}
...
function addDays(date, days) {
  ...
  const initialDateISO = date.toISOString();

  date.setDate(date.getDate() + days);

  const resultingDateISO = date.toISOString();

  return {
    initialDate: initialDateISO,
    daysAdded: days,
    resultingDate: resultingDateISO,
  };
}
...
function getDaysInMonth(date) {
  ...
  return { inputDate: initialDateISO, daysInMonth: daysInMonth };
}
...
function getFormattedTime(date) {
  ...
  return { inputDate: initialDateISO, formattedTime: formattedTime };
}
...
function compareDates(date1, date2) {
  ...
  return { date1: date1, date2: date2, comparisonResult: comparisonResult };
}
